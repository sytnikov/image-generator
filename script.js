...
async function generateImages(input) {
  ...
  for (let i = 0; i < imageCount; i++) {
    ...
    try {
      // Call your server's endpoint
      const response = await fetch("https://image-generator-proxy-server.vercel.app/generate-images", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ prompt }), // Send the prompt to your server
      });
